...
async def execute(interaction: discord.Interaction, event_id: int, game_id: int = None):
    ...
    try:
        ...
        # Run vote insert and counts in a single transaction
        async with db.transaction() as executor:
            existing_vote = await executor.fetchone(
                "SELECT 1 FROM votes WHERE event_id = ? AND game_id = ? AND user_id = ?",
                (event_id, game_id, interaction.user.id),
            )

            if existing_vote:
                await send_error_message(
                    interaction, "You have already voted for this event."
                )
                return
            ...
            player_count_row = await executor.fetchone(
            ...
            vote_count_row = await executor.fetchone(
        ...
        if player_count <= 3:
            ...
            consensus_threshold = max(
                2, math.ceil(player_count * VOTE_CONSENSUS_THRESHOLD)
            )
        ...
        if vote_count >= consensus_threshold:
            ...
            if winners:
                await announce_winners(
                    interaction.channel, winners, game["title"], interaction.client
                )
        else:
            await interaction.response.send_message(
                f"You voted for event {event_id} ({event['description']}). "
        ...
        try:
            ...
            logger.error(
                f"Failed to send error message: {type(response_error).__name__}: {str(response_error)}"
            )
        raise
