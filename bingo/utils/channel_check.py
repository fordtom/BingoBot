...
def _get_allowed_channel_id() -> int | None:
    return get_allowed_channel_id()


# Cached allowed channel id value, refreshed per check to allow runtime updates
ALLOWED_CHANNEL_ID = _get_allowed_channel_id()
...
async def is_allowed_channel(interaction: discord.Interaction) -> bool:
    ...
    channel_mention = (
        ...
        if channel
        else f"the designated channel (ID: {ALLOWED_CHANNEL_ID})"
    )
